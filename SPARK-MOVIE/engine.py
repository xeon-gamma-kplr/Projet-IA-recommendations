...
def create_parquet_files(spark):
    schema = StructType([ 
    ...
    df_movies = spark.read.option("header","true").schema(schema).csv("/workspace/Projet-IA-recommendations/SPARK-MOVIE/csv/movies.csv")
    # df_movies = df_movies.withColumn("Genres", regexp_replace('Genres', "|",","))
    df_movies.write.mode("overwrite").parquet("/workspace/Projet-IA-recommendations/SPARK-MOVIE/parquets/movies.parquet")
    ...
    df_ratings = spark.read.option("header","true").schema(schema).csv("/workspace/Projet-IA-recommendations/SPARK-MOVIE/csv/ratings.csv")
    df_ratings.write.mode("overwrite").parquet("/workspace/Projet-IA-recommendations/SPARK-MOVIE/parquets/ratings.parquet")
    # df_ratings = spark.read.parquet("/workspace/Projet-IA-recommendations/parquets/ratings.parquet")

def load_parquet_file(spark,path):
    try:
...
class Model:
    ...
    def save_model(self):
        print("save model...")
        self.model.write().overwrite().save("/workspace/Projet-IA-recommendations/SPARK-MOVIE/model/als_model")
...
class Data:
    def __init__(self, spark,ratings_path, movie_path):
        self.rating_path = ratings_path
        self.ratings = load_parquet_file(spark,ratings_path)
        self.movies = load_parquet_file(spark,movie_path)
    ...
    def save(self,spark):
        print("saving datas...")
        self.ratings.write.mode("overwrite").parquet("/workspace/Projet-IA-recommendations/SPARK-MOVIE/parquets/ratings.parquet")
        print("reload datas..")
        self.ratings = load_parquet_file(spark,self.rating_path)
        self.cache(self.ratings)
